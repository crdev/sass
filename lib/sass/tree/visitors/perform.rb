...
class Sass::Tree::Visitors::Perform < Sass::Tree::Visitors::Base
  ...
  def visit_prop(node)
    ...
    node.value_source_range = val.source_range if val.respond_to?(:source_range) && val.source_range
    node.value_original_filename = val.filename if val.respond_to?(:filename) && val.filename
    yield
  end
  ...
  def visit_variable(node)
    ...
    if node.expr.respond_to?(:source_range) && node.expr.source_range
